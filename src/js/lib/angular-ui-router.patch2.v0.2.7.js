...
Tim Hon 
Change log
1. Added patch to fix a modal dialog box. That patch is checked into the file called "patch2-modal.diff"
   which can be seen here: 
		https://github.com/besnik81/ui-router/compare/patch-2#diff-544b7ce9751e1a566e17d486dd4ab576R136

** NOTE - this fix didnt work, commented out for now **
2. There is a bug when a view gets instantiate twice. Did the fix as described here:
		https://github.com/angular-ui/ui-router/issues/881
	That diff is saved to "patch-two-instantiations.diff"
...
function $ViewDirective($state, $compile, $controller, $injector, $anchorScroll, $document) {
  ...
  var directive = {
	...
	// this was supposed to fix it, but didnt.
	//		initialView = angular.element($document[0].createComment(' ui-view-anchor '));		//CHG: patch
